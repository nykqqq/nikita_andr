...
int main() {
	setlocale(LC_ALL, "RU");
	int unsigned a, b, h; // unsigned тип положительных данных,т.к у трапеции не может быть отрицательных значений
	...
	if (a > 1000 or b > 1000) {
		...
		std::cout << "min: " << pow(2,sizeof(int)*8) << "\n" // формула для вывода в макс. и мин. значения выбранного типа данных
			<< "max: " << pow(2, sizeof(int) * 8)-1;// pow фунция для возведения в степень
	}
}
